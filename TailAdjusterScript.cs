...
public class TailAdjusterScript : MonoBehaviour
{
    ...
    public float propSpeed;                        // turning speed of propeller
    ...
    private void ElevatorAileronAdj()
    {
        ...
        {
            propSpeed = Mathf.Lerp(propSpeed, (planeScript.speedRatio + 2f) * 1000f, 0.2f * Time.deltaTime);
            propellerTr.Rotate(0f, 0f, propSpeed * Time.deltaTime, Space.Self);
        ...
        else if (tip == 1)     // if it is Lagg-03 plane
        {
            propSpeed = Mathf.Lerp(propSpeed, (planeScript.speedRatio + 2f) * 1000f, 0.2f * Time.deltaTime);
            propellerTr.Rotate(0f, 0f, -1f* propSpeed * Time.deltaTime, Space.Self);                                    // -1f multiplication since rotation in opposite dir
